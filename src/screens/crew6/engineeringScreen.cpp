...
void EngineeringScreen::onDraw(sp::RenderTarget& renderer)
{
    if (my_spaceship)
    {
        ...
        if (my_spaceship -> selected_system != SYS_None)
        {
            ...
            float health_max = my_spaceship->getSystemHealthMax(my_spaceship -> selected_system);
            if (health_max < 1.0f)
...
void EngineeringScreen::onUpdate()
{
    if (my_spaceship && isVisible())
    {
        ...
        if (my_spaceship -> selected_system != SYS_None)
        {
            ...
            {
                power_slider->setValue(my_spaceship->systems[my_spaceship -> selected_system].power_request + power_adjust);
                my_spaceship->commandSetSystemPowerRequest(my_spaceship -> selected_system, power_slider->getValue());
            }
            ...
            {
                coolant_slider->setValue(my_spaceship->systems[my_spaceship -> selected_system].coolant_request + coolant_adjust);
